...
const EXAMPLE_REPOSITORY_URL = 'https://github.com/PSPDFKit/nutrient-document-authoring-ai-example';

const getAiWriteMode = (useCaseId: AiUseCaseId, editorMode: ReturnType<DocAuthEditor['getEditorMode']>) => {
	if (useCaseId === 'template-fields') {
		return 'apply';
	}
	if (useCaseId === 'proofreading' || useCaseId === 'translation') {
...
const USE_CASES: readonly UseCaseUi[] = [
	...
	},
	{
		id: 'proofreading',
		label: 'Proofreading',
		documentUrl: '/proofreading-sample.docx',
		docxDocument: true,
	...
	},
	{
		id: 'translation',
		label: 'Translation',
		documentUrl: '/translation-sample.docx',
		docxDocument: true,
		...
		async onToolCall({ toolCall }) {
			...
			try {
				...
				const executed = await instance.toolkit.executeTool(
					parsedToolCall,
					writesDocument
						? { writeMode: getAiWriteMode(useCaseId, editorMode), reviewComments: activeReviewComments }
						: undefined,
				);
	...
	const runWorkflow = async ({ task, scope }: { task: string; scope: 'auto' | 'document' | 'selection' }) => {
		...
		try {
			...
			await instance.toolkit.applyWorkflowOutput(workflow, output, {
				...
				writeMode: getAiWriteMode(useCaseId, editorMode),
			});
	...
	return (
		<div className="app-frame">
			...
			<section className="use-case-header" aria-label="Use cases">
				<a href={EXAMPLE_REPOSITORY_URL} className="example-repository-link" target="_blank" rel="noreferrer">
					View on GitHub
				</a>
